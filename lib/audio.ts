...
class SoundEffects {
  ...
  // Realistic dual-tone automotive horn: F4 (349 Hz) & A4 (440 Hz)
  playHorn() {
    this.init();
    if (!this.ctx) return;
    const now = this.ctx.currentTime;

    const tones = [349.23, 440.0, 698.46];
    tones.forEach((freq, idx) => {
      const osc = this.ctx!.createOscillator();
      const gain = this.ctx!.createGain();
      const filter = this.ctx!.createBiquadFilter();

      osc.type = idx === 2 ? 'sine' : 'sawtooth';
      ...
      gain.gain.linearRampToValueAtTime(idx === 2 ? 0.08 : 0.22, now + 0.02);
      gain.gain.setValueAtTime(idx === 2 ? 0.08 : 0.22, now + 0.3);
